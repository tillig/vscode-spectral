...
namespace Is {
   ...
   * @param value The value to check/convert to string.
   */
 ...
 * @param severity - The Spectral diagnostic severity to convert.
 * @returns The converted severity for a VS Code diagnostic.
 */
 ...
 * @param documentOrUri The location of the document for which the path should be determined.
 * @returns A string value with the folder path if it can be determined; or undefined if not.
 */
 ...
 * @param problem - The Spectral rule result to convert to a VS Code diagnostic message.
 * @returns The converted VS Code diagnostic to send to the client.
 */
 ...
 * @param document - The document for which configuration settings should be determined.
 * @returns The lint settings for the specified document.
 */
function resolveSettings(document: TextDocument): Thenable<TextDocumentSettings> {
  ...
  resultPromise = connection.workspace.getConfiguration({ scopeUri: uri, section: '' })
    .then(async (configuration: ExtensionSettings) => {
      ...
      // filesystem path.
      let docPath = getDocumentPath(document.uri);
 ...
 * @param uri The document being validated when the error occurred.
 * @param err The error that occurred.
 */
function showErrorMessage(uri: string, err: any) {
...
function validate(document: TextDocument): Thenable<void> {
  return resolveSettings(document).then(async (settings) => {
    ...
    try {
      let results = await linter.lint(document, settings.ruleset);
